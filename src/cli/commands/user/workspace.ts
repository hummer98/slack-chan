...
/**
 * Resolve the target `team_id` for this `user` invocation.
 *
...
export async function resolveWorkspace(ctx: CommandContext, effects: Effects): Promise<string> {
  if (ctx.workspace !== null && ctx.workspace.length > 0) {
    try {
      ...
      throw new UserError(
        `user: --workspace must match /^T[A-Z0-9]{1,32}$/, got '${ctx.workspace}'.`,
      );
    }
  ...
  if (def === null) {
    throw new UserError(
      "user: no --workspace specified and no default_workspace is configured. " +
...
/** Load access token for `team_id` from the configured backend. */
export async function loadToken(
  ...
  try {
    ...
    throw new UserError(
      `user: cannot use ${kind} token backend on this platform (${msg}). ` +
  ...
  if (token === undefined) {
    throw new UserError(
      `user: no token stored for ${team_id} in ${kind} backend. ` +
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e);
    throw new UserError(`user: stored token violates AUP (${msg}).`);
  }
  return token;
}
