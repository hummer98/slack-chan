...
/**
 * Resolve the target `team_id` for this `search` invocation.
 *
...
export async function resolveWorkspace(ctx: CommandContext, effects: Effects): Promise<string> {
  if (ctx.workspace !== null && ctx.workspace.length > 0) {
    try {
      ...
      throw new UserError(
        `search: --workspace must match /^T[A-Z0-9]{1,32}$/, got '${ctx.workspace}'.`,
      );
    }
  ...
  if (def === null) {
    throw new UserError(
      "search: no --workspace specified and no default_workspace is configured. " +
...
export async function loadToken(
  ...
  try {
    ...
    throw new UserError(
      `search: cannot use ${kind} token backend on this platform (${msg}). ` +
  ...
  if (token === undefined || token.length === 0) {
    throw new UserError(
      `search: no token stored for ${team_id} in ${kind} backend. ` +
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e);
    throw new UserError(`search: stored token violates AUP (${msg}).`);
  }
  return token;
}
